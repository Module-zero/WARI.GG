...
## 관리자 서비스 
 ✏️ Written By 잼성주성이
- 관리자 서비스는 **게시판 관리, 게시글 관리, 신고 관리** 서비스 가 있다.
관리자 페이지는 별도의 웹 화면으로 보여지며 관리자 계정으로 로그인하여 서비스를 이용할 수 있다.
관리자 계정으로 로그인 시 위의 3가지 서비스 탭 중 하나를 선택하여 해당 서비스를 이용할 수 있다.

-> 게시판 관리 서비스(구현 순위 하)
관리자는 게시판을 생성/수정/삭제 할 수 있다. 
게시판 생성 시에는 게시판 제목을 입력한 뒤 게시판을 생성할 수 있다.
게시판 수정 시에는 게시판 제목을 수정한 뒤 다시 저장할 수 있다.
게시판 삭제 시에는 해당 게시판과 모든 게시글을 일괄적으로 삭제할 수 있다.
...
-> 신고 관리 서비스(구현 순위 상)
신고 관리 서비스는 게시판의 형태로 관리자에게 제공된다.
관리자는 해당 게시판에서 신고 접수 게시글들을 열람할 수 있다.
...
그리고 관리자에게는 아래와 같이 2가지 기능이 제공된다.
  - 신고받은 한줄평 블라인드 처리
  - 신고받은 계정 사용금지 조치

관리자는 신고받은 한줄평의 내용을 확인한 뒤 해당 한줄평을 블라인드 처리할 수 있다.
사용금지는 신고받은 사용자의 누적 신고 회수에 따라 3일/15일/30일/영구정지 중 하나를 택할 수 있다.
신고접수 게시글은 '미완료/완료' 탭으로 나뉘어져 있으며 오래된 게시글 순으로 정렬되어 나온다.


<논의사항>
- 관리자 계정은 여러개?
- 게시판 생성/수정/삭제 기능의 필요성(빈번히 일어나지 않기에 별도의 기능이 필요한지, css 수정 문제)
- 게시판의 유형과 맞지 않는 게시글을 관리자가 직접 판단할 것인지 아니면 사용자가 글을 게시하는 과정에서 걸러낼 것인지
  -> 만약 관리자가 일반회원의 게시글을 임의로 삭제할 경우 사용자에게 별도의 알림을 줄것인지
- 한줄평 블라인드 처리는 1. 관리자가 직접 할 것인지 2. 신고가 누적되면 자동으로 될 것인지
  -> 2번 방식을 택할 경우 하나의 한줄평에도 별도의 신고회수가 저장되어야함
- 한줄평 뿐만 아니라 댓글에 한해서도 신고기능을 넣을 것인지
- 관리자 계정의 어플 로그인을 가능하게 할 것인지
- 관리자 웹페이지를 위한 서버가 별도로 필요한지
