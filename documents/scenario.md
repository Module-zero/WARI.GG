...
-----
 ...
 ## 챔피언 정보 제공 서비스  
 ✏️ Written By 김우석    
 - 사용자에게 **와일드리프트 챔피언에 대한 정보를 제공** 하는 서비스

- 챔피언 정보에는 챔피언 역할군, 레벨 별 능력치, 스킬 정보, 추천 룬과 스펠, 추천 라인과 아이템 트리, 출시된 스킨, 챔피언 설명, 카운터픽 및 한줄평이 있다.
>> 사용자는 해당 챔피언에 대한 한줄평과 카운터픽을 작성할 수 있으며, 다른 사용자가 작성한 글에 답글 및 좋아요를 달 수 있으며 신고를 할 수 있다.
카운터픽 투표창이 존재하며, 현재 패치 버전에 따른 카운터픽을 1개 고를 수 있으며 수정이 가능하다. 또, 많은 사용자가 뽑은 카운터픽은 패치 버전별로 득표순으로 3위까지 챔피언창에 표시된다.

챔피언 정보에서는 와일드 리프트에 기본 저장된 아이템 트리를 보여주고 공략 게시판과 연결되어 공략 게시글의 좋아요 순으로 최대 3,,5,,10개까지 보여준다.

챔피언 역할군은 암살자, 전사, 마법사, 원거리 딜러, 서포터, 탱커로 분류된다.

능력치는 공격력, 체력, 마나(기력, 분노 등), 방어력, 마법 저항력, 공격속도, 체력 재생, 마나 재생, 이동속도, 사거리로 분류된다.

스킬 정보는 패시브, 1, 2, 3번 스킬과 궁극기에 대한 쿨타임, 데미지, 스킬 설명을 의미한다.

룬은 감전, 콩콩이, 정복자, 기민한 발놀림,    착취, 여진, 생명의 샘, 도벽 총 8개의 핵심룬 중 하나와 지배, 결의, 영감에서 세부 룬을 하나씩 고를 수 있다.
지배 : 난폭, 폭풍의 결집, 사냥꾼 – 흡혈, 승전보, 약점, 챔피언
결의 : 근성, 사전 준비, 사냥꾼 – 거인, 재생, 충성, 정령 주술사
영감 : 개척자, 지휘자, 사냥꾼 – 비범함, 달콤함, 집단 사냥, 마나순환 팔찌

스펠은 점화, 강타, 점멸, 탈진, 유체화, 회복, 방어막 7개로 분류된다.

라인은 바론(탑), 정글, 미드, 드래곤(바텀), 서포터 5개로 분류된다.
