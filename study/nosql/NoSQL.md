...
## 요즈음 데이터의 특징  
- **대규모, 단순한 형태**   
 ▶︎▹ ex) SNS의 경우 대용량의 데이터가 발생하지만 단순한 형태이다.  
 ▶︎▹ 대규모 데이터이므로 **관계형 데이터(RDBMS) 로는 저장하는데 문제** 가 있다.
...
>P – Partitioning( 부분 결함 용인)  
 ...
 ▶︎▹ NoSQL 는 **(C or A) & P** 를 만족한다.

## NoSQL ( Not Only SQL)
 ▶︎▹ 기존 RDBMS 의 단점을 보완해주는 형태  
 ▶︎▹ Foreign key로 정의 및 join 등의 관계 연산을 하지 않는다.  
 ▶︎▹ RDBMS와 는 다르게 스키마가 유동적이다.  
      ▶︎▹ 데이터를 **분산해서 저장** , 데이터를 상호 복제  
...
- 저장되는 Value 타입은 **Document  라는 구조화된 데이터** 타입 (JSON, XML, YAML …)  
 ▶︎▹ ```USER : { name : “jeonka”, age : “26” }```

#### NoSQL 설계시 주의사항
-	내가 가지고 있는 **질문(기능)을 기준** 으로 모델링 한다  
